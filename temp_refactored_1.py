...
def compare_shape_fill(shape1, shape2, ctx):
    """Compare shape fill color and transparency"""
    from pptx.enum.dml import MSO_FILL_TYPE
    
    if not hasattr(shape1, 'fill') or not hasattr(shape2, 'fill'):
        return True  # Can't compare if no fill attribute
    ...
    if type1 != type2:
        log_mismatch(f"{ctx}: Fill type mismatch - test={type1}, golden={type2}")
        return False
    
    # If both are SOLID fills, compare colors
    if type1 == MSO_FILL_TYPE.SOLID:
        try:
            color1 = fill1.fore_color
            color2 = fill2.fore_color
            
            # Check color type first
            if color1.type != color2.type:
                log_mismatch(f"{ctx}: Fill color type mismatch - test={color1.type}, golden={color2.type}")
                return False
            
            # If RGB colors, compare them
            if hasattr(color1, 'rgb') and color1.rgb is not None and \
               hasattr(color2, 'rgb') and color2.rgb is not None:
                if color1.rgb != color2.rgb:
                    log_mismatch(f"{ctx}: Fill color mismatch - test={color1.rgb}, golden={color2.rgb}")
                    return False
            
            # If theme colors, compare them
            if hasattr(color1, 'theme_color') and hasattr(color2, 'theme_color'):
                if color1.theme_color != color2.theme_color:
                    log_mismatch(f"{ctx}: Fill theme color mismatch - test={color1.theme_color}, golden={color2.theme_color}")
                    return False
                    
            # Compare brightness if available
            if hasattr(color1, 'brightness') and hasattr(color2, 'brightness'):
                if color1.brightness != color2.brightness:
                    log_mismatch(f"{ctx}: Fill brightness mismatch - test={color1.brightness}, golden={color2.brightness}")
                    return False
                    
        except Exception as e:
            log_mismatch(f"{ctx}: Fill color comparison error - {str(e)}")
            return False
    
    # Check transparency via XML (works for all fill types)
    try:
        ns = {'a': 'http://schemas.openxmlformats.org/drawingml/2006/main'}
        
        # Find alpha in solidFill
        alpha1_elem = shape1._element.find('.//a:solidFill//a:alpha', ns)
        alpha2_elem = shape2._element.find('.//a:solidFill//a:alpha', ns)
        
        alpha1 = alpha1_elem.get('val') if alpha1_elem is not None else None
        alpha2 = alpha2_elem.get('val') if alpha2_elem is not None else None
        
        if alpha1 != alpha2:
            pct1 = round(100 - (int(alpha1) / 1000), 1) if alpha1 else 0
            pct2 = round(100 - (int(alpha2) / 1000), 1) if alpha2 else 0
            log_mismatch(f"{ctx}: Fill transparency mismatch - test={pct1}%, golden={pct2}%")
            return False
    except Exception:
        pass  # Transparency check is optional
...
def compare_pptx_files(file1_path, file2_path):
    ...
    for slide_idx, (slide1, slide2) in enumerate(zip(prs1.slides, prs2.slides), 1):
        ...
        for test_idx, test_shape in enumerate(test_shapes):
            ...
            # Validate shape fill and outline (for all shapes with these properties)
