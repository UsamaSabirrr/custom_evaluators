...
# ============================================================================
# ANIMATION EXTRACTION AND COMPARISON
# ============================================================================
def extract_animation_data(pptx_path):
    """Extract animation information from a PPTX file by parsing XML"""
    animations_by_slide = []
    ...
    try:
        with zipfile.ZipFile(pptx_path, 'r') as zip_ref:
            ...
            slide_files = [f for f in zip_ref.namelist() 
                          ...
                          and f.endswith('.xml') 
                          and 'slideLayout' not in f 
                          and 'slideMaster' not in f
                          and '_rels' not in f]
            ...
            for slide_file in slide_files:
                ...
                for shape in slide_root.findall('.//p:sp', namespaces):
                    ...
                    if nvSpPr is not None:
                        ...
                        if cNvPr is not None:
                            ...
                            if shape_id:
                                shape_id_map[shape_id] = shape_name or f"Shape_{shape_id}"
                
                # Look for timing/animation data in the slide XML itself
                timing = slide_root.find('.//p:timing', namespaces)
                
                if timing is not None:
                    seq_idx = 0
                    
                    # Parse animation sequences
                    for seq in timing.findall('.//p:seq', namespaces):
                        for cTn in seq.findall('.//p:cTn', namespaces):
                            for par in cTn.findall('.//p:par', namespaces):
                                for child_cTn in par.findall('.//p:cTn', namespaces):
                                    # Extract timing and trigger
                                    duration = child_cTn.get('dur', 'auto')
                                    preset_class = child_cTn.get('presetClass', '')
                                    preset_id = child_cTn.get('presetID', '')
                                    ...
                                    if stCondLst is not None:
                                        ...
                                        if cond is not None:
                                            ...
                                            if evt == 'onNext':
                                                ...
                                                trigger = 'onClick'
                                            elif evt == 'begin' or delay == '0':
                                                trigger = 'withPrevious'
                                            else:
                                                trigger = 'afterPrevious'
                                    
                                    # Find animation effect
                                    for anim_effect in child_cTn.findall('.//p:animEffect', namespaces):
                                        transition = anim_effect.get('transition', 'in')
                                        filter_val = anim_effect.get('filter', '')
                                        
                                        # Get actual duration from nested cTn inside cBhvr
                                        actual_duration = duration
                                        cBhvr = anim_effect.find('.//p:cBhvr', namespaces)
                                        if cBhvr is not None:
                                            nested_cTn = cBhvr.find('.//p:cTn', namespaces)
                                            if nested_cTn is not None:
                                                actual_duration = nested_cTn.get('dur', duration)
                                        
                                        # Get target shape
                                        tgtEl = anim_effect.find('.//p:tgtEl', namespaces)
                                        target_shape_id = None
                                        target_name = 'Unknown'
                                        
                                        if tgtEl is not None:
                                            spTgt = tgtEl.find('.//p:spTgt', namespaces)
                                            if spTgt is not None:
                                                target_shape_id = spTgt.get('spid')
                                                target_name = shape_id_map.get(target_shape_id, f"Shape_{target_shape_id}")
                                        
                                        anim_info = {
                                            'seq_index': seq_idx,
                                            'target_id': target_shape_id,
                                            'target_name': target_name,
                                            'effect_type': filter_val or preset_class or 'appear',
                                        ...
                                        slide_animations.append(anim_info)
                                        seq_idx += 1
                                    
                                    # Find set animation (skip - usually paired with effects)
                                    for anim_set in child_cTn.findall('.//p:set', namespaces):
                                        continue
                                    
                                    # Find animMotion
                                    for anim_motion in child_cTn.findall('.//p:animMotion', namespaces):
                                        origin = anim_motion.get('origin', 'layout')
                                        path = anim_motion.get('path', '')
                                        
                                        tgtEl = anim_motion.find('.//p:tgtEl', namespaces)
                                        target_shape_id = None
                                        target_name = 'Unknown'
                                    ...
                                    # Find anim (color/scale/rotation changes)
                                    for anim_base in child_cTn.findall('.//p:anim', namespaces):
                                        calcmode = anim_base.get('calcmode', 'lin')
                                        
                                        tgtEl = anim_base.find('.//p:tgtEl', namespaces)
                                        target_shape_id = None
                                        target_name = 'Unknown'
                ...
                animations_by_slide.append(slide_animations)
    
    except Exception as e:
        log_mismatch(f"ERROR extracting animations: {str(e)}")
    
    return animations_by_slide


def compare_animations(anims1, anims2, slide_idx):
    """Compare animations between two slides"""
    if len(anims1) != len(anims2):
        log_mismatch(f"Slide {slide_idx} (ANIMATIONS): Count mismatch - test={len(anims1)}, golden={len(anims2)}")
        return False
    
    for idx, (anim1, anim2) in enumerate(zip(anims1, anims2)):
        ctx = f"Slide {slide_idx}, Animation {idx + 1}"
        
        # Compare target (by name if available, otherwise by ID)
        if anim1.get('target_name') != anim2.get('target_name'):
            log_mismatch(f"{ctx}: Target mismatch - test={anim1.get('target_name')}, golden={anim2.get('target_name')}")
            return False
        
        # Compare effect type (includes direction like "blinds(horizontal)")
        if anim1.get('effect_type') != anim2.get('effect_type'):
            log_mismatch(f"{ctx}: Effect type mismatch - test={anim1.get('effect_type')}, golden={anim2.get('effect_type')}")
            return False
        
        # Compare trigger (onClick, withPrevious, afterPrevious)
        if anim1.get('trigger') != anim2.get('trigger'):
            log_mismatch(f"{ctx}: Trigger mismatch - test={anim1.get('trigger')}, golden={anim2.get('trigger')}")
            return False
        
        # Compare duration
        if anim1.get('duration') != anim2.get('duration'):
            log_mismatch(f"{ctx}: Duration mismatch - test={anim1.get('duration')}, golden={anim2.get('duration')}")
            return False
        
        # Compare delay
        if anim1.get('delay') != anim2.get('delay'):
            log_mismatch(f"{ctx}: Delay mismatch - test={anim1.get('delay')}, golden={anim2.get('delay')}")
            return False
        
        # Compare transition (in/out)
        if anim1.get('transition') != anim2.get('transition'):
            log_mismatch(f"{ctx}: Transition mismatch - test={anim1.get('transition')}, golden={anim2.get('transition')}")
            return False
        
        # Compare preset_class if present (entr, exit, emph, path)
        if anim1.get('preset_class') != anim2.get('preset_class'):
            log_mismatch(f"{ctx}: Preset class mismatch - test={anim1.get('preset_class')}, golden={anim2.get('preset_class')}")
            return False
        
        # Compare additional properties if present
        if 'path' in anim1 and 'path' in anim2:
            if anim1['path'] != anim2['path']:
                log_mismatch(f"{ctx}: Path mismatch")
                return False
    
    return True


def check_pptx_animations(file1_path, file2_path):
    """Check if animations match between two PPTX files"""
    try:
        test_anims = extract_animation_data(file1_path)
        golden_anims = extract_animation_data(file2_path)
        
        # Check if number of slides match
        if len(test_anims) != len(golden_anims):
            log_mismatch(f"Animation slide count mismatch - test={len(test_anims)}, golden={len(golden_anims)}")
            return False
        
        # Compare animations for each slide
        for slide_idx in range(len(test_anims)):
            slide_num = slide_idx + 1
            if not compare_animations(test_anims[slide_idx], golden_anims[slide_idx], slide_num):
                return False
        
        return True
    
    except Exception as e:
        log_mismatch(f"Animation check failed: {str(e)}")
        return False
...
def compare_pptx_files(file1_path, file2_path):
    ...
    # ============================================================
    # ANIMATION CHECK - MANDATORY
    # ============================================================
    if not check_pptx_animations(file1_path, file2_path):
        return 0
...
def main():
    ...
    print("=" * 70)
    log_message(f"{Colors.BOLD}PPTX Evaluator Test Suite v4 (with Animations){Colors.RESET}", Colors.CYAN)
    print("=" * 70)
